...
import '../styles/home.css'; // <- make sure path matches your project
...
function TicketModal({ open, onClose, item }) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (!resp.ok) {
        // show text response or fallback
      ...
      // fallback: in dev environment mock success
...
function Hero({ onGetTickets }) {
  ...
  const nextEventDate = new Date('2025-10-26T10:00:00'); // update as needed
    ...
    <section id="home" className="hero">
      <div className="container hero-grid">
        ...
        <aside className="hero-card" aria-label="Next event">
          ...
          <div className="meta">
            <span>{nextEventDate.toLocaleString(undefined, { weekday: 'short', month: 'short', day: 'numeric' })} • {nextEventDate.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
  ...
  return (
    <section id="showcase" className="section container">
      <div className="featured-top">
        <div className="featured-carousel" onMouseEnter={() => { setPaused(true); stopAuto(); }} onMouseLeave={() => { setPaused(false); startAuto(); }}>
          ...
          <div className="carousel-indicators" role="tablist" aria-label="Slide indicators">
            {FEATURED_CARS.map((_, i) => (
              <button key={i} className={`dot ${i === index ? 'active' : ''}`} onClick={() => { setIndex(i); stopAuto(); }} aria-label={`Go to slide ${i + 1}`} />
            ))}
          </div>
        </div>
...
export default function Home() {
  ...
  return (
    <>
      <Navbar />
      <main>
        <Hero onGetTickets={() => openTicket(null)} />
        <FeaturedSection onOpenTicket={openTicket} />
          ...
          <div className="events-grid">
            <article className="event-card">
              ...
              <div className="event-body">
                ...
                <div className="event-actions">
                  <button className="btn" onClick={() => openTicket(null)}>Book Tickets</button>
            ...
            <article className="event-card">
              ...
              <div className="event-body">
                ...
                <div className="event-actions">
                  <button className="btn" onClick={() => openTicket(null)}>Get Tickets</button>
                  <button className="btn secondary" onClick={() => alert('Event details (stub)')}>Details</button>
                </div>
              </div>
            </article>
